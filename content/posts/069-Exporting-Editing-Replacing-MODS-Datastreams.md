...
lastmod: 2020-04-17T12:57:11-05:00
draft: false
tags:
  - MODS
  - Export
...
The transition to distance learning and social distancing that's taken place at _Grinnell College_ in the wake of the _COVID-19_ pandemic may afford _GC Libraries_ an opportunity to do some overdue and necessary metadata cleaning in [Digital.Grinnell](https://digital.grinnell.edu).

# Process Summary

This turned out to be a much more difficult undertaking than I imagined, but as of mid-April, 2020, I have a 10-part workflow that actually works.  The ten parts are:
   ...
   4. Creation and execution of the _Map-MODS-to-MASTER_ _Python 3_ script on iMac _MA8660_ to create a `<collection>.csv` and accompanying `.log` files for each _Digital Grinnell_ collection.
   ...
   6. Creation of the _Drupal 8_ local _lando-d8_ platform and the new _mods-via-twig_ _Drush_ command on my personal Mac Mini.
   7. Copying via `rsync` of all modified `<collection>.csv` files from `//STORAGE/LIBRARY/ALLSTAFF/DG-Metadata-Review-2020-r1` to my local _lando-d8_ platform.
   8. Execution of the aforementioned _mods-via-twig_ command in _lando-d8_ to generate updated `grinnell_<PID>_MODS.xml` files on a collection-by-collection basis.
...
## Part 2 - Execution of _islandora\_datastream\_export_ to Dump MODS Datastreams
